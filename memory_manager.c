...
void temp_map_page(phyaddr addr) { //          0xFFF                1          2
	*((phyaddr*)TEMP_PAGE_INFO) = (addr & ~PAGE_OFFSET_MASK) | PAGE_VALID | PAGE_WRITABLE;
	asm("invlpg (,%0,)"::"a"(TEMP_PAGE));
}

//позволяет спроецировать указанный блок физических страниц по нужному виртуальному адресу,
//map_pages(kernel_page_dir(0x1000), idt, 0x8000, 1, PAGE_VALID | PAGE_WRITABLE);
bool map_pages(phyaddr page_dir, void *vaddr, phyaddr paddr, size_t count, unsigned int flags) {
	for (; count; count--) {
		...
		for (shift = PHYADDR_BITS - PAGE_TABLE_INDEX_BITS; shift >= PAGE_OFFSET_BITS; shift -= PAGE_TABLE_INDEX_BITS) {
			unsigned int index = ((size_t)vaddr >> shift) & PAGE_TABLE_INDEX_MASK; //3ff
...
//узнать куда же будет отображён виртуальный адрес и отображён ли он
//      get_page_info(kernel_page_dir(0x1000), (void*)0xB8000));
phyaddr get_page_info(phyaddr page_dir, void *vaddr) {
	phyaddr page_table = page_dir;
	char shift;//  32                10                   22         12                         10
	for (shift = PHYADDR_BITS - PAGE_TABLE_INDEX_BITS; shift >= PAGE_OFFSET_BITS; shift -= PAGE_TABLE_INDEX_BITS) {
		unsigned int index = ((size_t)vaddr >> shift) & PAGE_TABLE_INDEX_MASK; //3ff
		temp_map_page(page_table);
		if (shift > PAGE_OFFSET_BITS) {
...
phyaddr alloc_phys_pages(size_t count) {
	...
	if (free_phys_memory_pointer != -1) {
		...
		//printf("free_phys_memory_pointer = 0x%x\n", free_phys_memory_pointer);
		do {
			...
			if (((PhysMemoryBlock*)TEMP_PAGE)->size == count) {
				...
				//printf("1) free_phys_memory_pointer = 0x%x\n", free_phys_memory_pointer);
				//printf("2) curblock = 0x%x\n", cur_block);
				if (cur_block == free_phys_memory_pointer) {
					free_phys_memory_pointer = next;
					//printf("2) free_phys_memory_pointer = 0x%x\n", free_phys_memory_pointer);
					if (cur_block == free_phys_memory_pointer) {
				...
				/*printf("1) ((PhysMemoryBlock*)TEMP_PAGE)->size = 0x%x\n", ((PhysMemoryBlock*)TEMP_PAGE)->size);
				printf("2) offset = 0x%x\n", (((PhysMemoryBlock*)TEMP_PAGE)->size << PAGE_OFFSET_BITS));
				printf("2) free_memory_size( = 0x%x\n", get_free_memory_size());*/
				break;
			}
